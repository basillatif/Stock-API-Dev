#seed(1)
stockdat = read.csv("/Users/RebeccaFang/Downloads/googl_data.csv")
...
getSymbols("GOOGL",from="2012-11-01",to="2016-12-31")
...
GOOGL%>%chartSeries(TA='addBBands();addBBands(draw="p");addVo();addMACD()',subset='2016')
...
random_data%>%ggplot(aes(Day,Price))+geom_line()+labs(title="Google (GOOGL) price simulation for 2017")+theme_bw()
...
final_mat%>%gather("Simulation","Price",2:501)%>%ggplot(aes(x=Day,y=Price,Group=Simulation))+geom_line(alpha=0.2)+labs(title="Google Stock (GOOGL): 500 Monte Carlo Simulations for 2017")+theme_bw()
...
library(dplyr)
library(PerformanceAnalytics)
getSymbols("GOOGL",from="2017-01-01",to="2017-12-31")
GOOGL_log_returns1<-GOOGL%>%Ad()%>%dailyReturn(type='log')
GOOGL_mean_log1<-mean(GOOGL_log_returns1)
mean_log1<-round(GOOGL_mean_log1,4)
GOOGL_sd_Log1<-sd(GOOGL_log_returns1)
sd_log1<-round(GOOGL_sd_Log1,4)
graphic2<-data.frame(mean_log1,sd_log1)
rownames(graphic2)<-c("GOOGL")
...
GOOGL%>%Ad()%>%chartSeries()
GOOGL%>%chartSeries(TA='addBBands();addBBands(draw="p");addVo();addMACD()',subset='2017')

probs<-c(0.005,0.025,0.25,0.5,0.75,0.975,0.995)

GOOGL_dist1<-GOOGL_log_returns1%>%quantile(probs=probs,na.rm=TRUE)
GOOGL_mean1<-mean(GOOGL_log_returns1,na.rm=TRUE)
GOOGL_sd1<-sd(GOOGL_log_returns1,na.rm=TRUE)


GOOGL_mean1%>%exp() 

#random_data1<-getSymbols("GOOGL2",from="2017-01-01",to="2017-12-31")
#random_data1%>%ggplot(aes(Day,Price))+geom_line()+labs(title="Google (GOOGL1) real price for 2017")+theme_bw()